...
const DeleteCategory = () => {
  ...
  const handleEdit = (oldName, Name, id) => {
    axios
      .put(`http://localhost:8000/activity${id}`, {
        currentName: oldName,
        newName: Name,
      })
      .then((response) => {
  ...
  const handleEditActivity = (oldActivity, updatedActivity) => {
    axios
      .put(`http://localhost:8000/activity/${oldActivity.id}`, {
    ...
    <TouristNavBar />
      ...
      <Box sx={{ p: 6, maxWidth: 1200 }}>
        <Box sx={{ display: "flex", justifyContent: "center", mb: 3 }}>
          <Typography variant="h4">Available Categories</Typography>
        </Box>
        {showTextField && (
          <Box sx={{ display: "flex", justifyContent: "center", mb: 3 }}>
            <TextField
