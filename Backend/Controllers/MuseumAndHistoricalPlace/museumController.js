...
const getAllMyMuseums = async (req, res) => {
    ...
    try {
        ...
        const museum = await museumModel.find({ createdBy });
...
const filterByTags = async (req, res) => {
    try {
        ...
        const results = await museumModel.find({
           // 'tags.museumTag': { $in: tagsArray }
           'tags': { $in: tagsArray }
            // Matches documents that contain any of the specified tags
        });

        // const results = await museumModel.find({
        //     tags: {
        //         $elemMatch: {
        //             museumTag: { $in: tagsArray } // Check if any museumTag is in tagsArray
        //         }
        //     }
        // });
        ...
        // const results = await museumModel.aggregate([
        //     {
        //         $match: {
        //             // Check if any museumTag in tags is in tagsArray
        //             tags: {
        //                 $elemMatch: {
        //                     museumTag: { $in: tagsArray } // Matches if any museumTag is in the tagsArray
        //                 }
        //             }
        //         }
        //     }
...
//         // Ensure tags are provided and is an array
//         if (!tags) {
//             return res.status(400).json({ message: "Tags are required" });
//         }
...
//         // Step 1: Find the museumTag IDs that match the tagsArray
//         const matchingTags = await museumTagModel.find({
//             museumTag: { $in: tagsArray }
//         }).select('_id'); // Only select the IDs

//         // Extract the IDs from the results
//         const tagIds = matchingTags.map(tag => tag._id);
//         console.log('Matching Tag IDs:', tagIds);

//         // Step 2: Find museums that contain the matching tag IDs in the tags array
//         const results = await museumModel.find({
//             'tags': { $elemMatch: { _id: { $in: tagIds } } } // Use $elemMatch to match tag IDs
//         });
...
//         // Split the tags string into an array (if it's a comma-separated string)
//         const tagsArray = Array.isArray(tags) ? tags : tags.split(',').map(tag => tag.trim().toLowerCase()); // Normalize tags

//         console.log("Filtering by tags:", tagsArray); // Debugging line

//         // Build the query to filter by tags
//         const results = await museumModel.find({
//             "tags.museumTag": { $in: tagsArray } // Adjusted to match the nested structure
//         });
...
//         // Build the query to find museums and populate their tags
//         const museums = await museumModel.find().populate('tags'); // Populate the tags field

//         // Filter museums based on the tagsArray
//         const results = museums.filter(museum =>
//             museum.tags.some(tag => tagsArray.includes(tag.museumTag.toLowerCase())) // Compare with normalized tags
//         );
...
//         // Filter museums based on the tagsArray
//         const results = museums.filter(museum => 
//             museum.tags && // Check if museum.tags is defined
//             Array.isArray(museum.tags) && // Ensure museum.tags is an array
//             museum.tags.some(tag => tag.museumTag && tagsArray.includes(tag.museumTag.toLowerCase())) // Compare with normalized tags
//         );
